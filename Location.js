...
const Location = () => {
  ...
  useEffect(() => {
    setFlag(flags[countriefs.indexOf(selectedLanguage)])
  },[selectedLanguage])
  ...
  useEffect(() => {
    (async () => {
      ...
      let location = await Locationd.getCurrentPositionAsync({});
      setLocation(location);

      lat = location.coords.latitude
      lon = location.coords.longitude;
      console.log('here',lat,lon)
      fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lon}`).then(response =>
          response.json()
      ).then(data => {
...
function change(itemValue){
  ...
  setFlag(flags[countriefs.indexOf(itemValue)])
}
  return (
